...
public class CustomerInfoClient {
    ...
    public TransactionResponse getCustomerInfo(TransactionParam request) throws ReportingApiException{
        log.info("ReportingApiClient client call started: {}", request);
        try {
            String accessToken = Optional.ofNullable(loginClient.login()).map(LoginResponse::getToken).orElse(null);
            ...
            return Optional.of(restTemplate.postForEntity(reportingApiConfiguration.getClientUrl(), entity, TransactionResponse.class).getBody()).get();
        } catch (Exception e) {
            log.error("Check client service ", e);
